...
import { ElFormItem, ElSlider, ElInputNumber, ElButton } from "element-plus";
...
const indices = computed(() =>
  props.multiple && Array.isArray(props.modelValue)
    ? Array.from(Array(props.modelValue.length).keys())
    : []
);
const unarray = computed(() =>
    ...
    >
      <div v-for="index in indices" :key="index">
        <el-slider
          ...
          :max="max"
          :step="step"
          @input="(newValue) => onChanged(newValue, index)"
          :disabled="disabled"
          show-input
        />
        <el-input-number
          ...
          :max="max"
          :step="step"
          @change="(newValue) => onChanged(newValue, index)"
          :disabled="disabled"
        />
      </div>
      <div>
