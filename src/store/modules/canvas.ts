...
export const useCanvasStore = defineStore("canvas", () => {
  ...
  const maxDimensions = ref(1024);
  const minDimensions = ref(64);
  const params = ref({
    width: 512,
    height: 512,
  });
  const generatorImageProps = ref({
  ...
  const imageProps = computed(() =>
    usingInpainting.value ? inpainting.value : img2img.value
  );
  //   const generatorImageProps = computed(
  //     () => useGeneratorStore().currentImageProps
  //   );
  ...
  async function pathCreate({
    ...
    if (erase) {
      ...
      history.drawPath.stroke = draw
        ? drawColor.value
        : imageProps.value.maskPathColor;
    }
    let scaledDrawPath = (await asyncClone(history.drawPath)) as fabric.Path;
    scaledDrawPath = scaledDrawPath.scale(
      imageProps.value.imageScale
    ) as fabric.Path;
    scaledDrawPath.left =
      (scaledDrawPath.left as number) +
      (history.drawPath.left as number) * (imageProps.value.imageScale - 1);
    scaledDrawPath.top =
      (scaledDrawPath.top as number) +
      (history.drawPath.top as number) * (imageProps.value.imageScale - 1);
    if (draw) {
  ...
  function undoAction() {
    ...
    if (drawing.value) {
      ...
      imageProps.value.visibleImageLayer.remove(
        path.visibleDrawPath as fabric.Path
      );
    } else {
      imageProps.value.drawLayer.remove(path.drawPath as fabric.Path);
      imageProps.value.visibleDrawLayer.remove(
        path.visibleDrawPath as fabric.Path
      );
    }
  ...
  function newImage(image: fabric.Image) {
    ...
    if (
      width.value > maxDimensions.value ||
      height.value > maxDimensions.value
    ) {
      const { newHeight, newWidth } = scaleImageTo(
        image,
        width.value,
        height.value,
        maxDimensions.value
    ...
    if (
      width.value < minDimensions.value ||
      height.value < minDimensions.value
    ) {
      const { newHeight, newWidth } = scaleImageTo(
        image,
        width.value,
    ...
    image.cloneAsImage((clonedImage: fabric.Image) => {
      ...
      imageProps.value.imageLayer = makeInvisibleLayer({
        image: clonedImage,
        layerHeight: clonedImage.height,
        layerWidth: clonedImage.width,
      });
    });
    ...
    image.cloneAsImage((clonedImage: fabric.Image) => {
      if (!imageProps.value.canvas) return;
      if (
        width.value !== visibleDimensions ||
        height.value !== visibleDimensions
      ) {
        const { newHeight, newWidth } = scaleImageTo(
          clonedImage,
          width.value,
      ...
      params.value.width = scaledWidth - (scaledWidth % 64);
      params.value.height = scaledHeight - (scaledHeight % 64);
  ...
  function saveImages() {
    ...
    if (!imageProps.value.drawLayer) return;
    const cropX =
      imageProps.value.imageLayer.getCenter().left -
      (params.value.width as number) / 2;
    const cropWidth = params.value.width;
    const cropY =
      imageProps.value.imageLayer.getCenter().top -
      (params.value.height as number) / 2;
    const cropHeight = params.value.height;
    ...
    generatorImageProps.value.sourceImage =
      imageProps.value.imageLayer.toDataURL(dataUrlOptions);
    generatorImageProps.value.maskImage =
      imageProps.value.redoHistory.length === 0 || drawing.value
        ? undefined
        : imageProps.value.drawLayer.toDataURL(dataUrlOptions).split(",")[1];
  }
  ...
  function updateCropPreview() {
    ...
    imageProps.value.cropPreviewLayer = makeNewLayer({
      layerWidth: (params.value.width as number) / imageProps.value.imageScale,
      layerHeight:
        (params.value.height as number) / imageProps.value.imageScale,
  ...
  function resetDrawing() {
    ...
    if (drawing.value) {
      //   const store = useGeneratorStore();
      newBlankImage(params.value.height || 512, params.value.width || 512);
    }
    imageProps.value.drawLayer = undefined;
    imageProps.value.redoHistory = [];
    imageProps.value.undoHistory = [];
  ...
  function downloadMask() {
    ...
    if (drawing.value) {
      anchor.href =
        ...
        generatorImageProps.value.sourceImage?.split(",")[1];
      anchor.download = "image_drawing.webp";
      anchor.click();
      return;
    }
    anchor.href =
      "data:image/webp;base64," + generatorImageProps.value.maskImage;
